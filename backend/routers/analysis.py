...
def analyze_product_api(request: AnalysisRequest, db: Session = Depends(get_db)):
    """React에서 호출할 메인 분석 API 엔드포인트 (반환값 수정)"""
    try:
        # 1. DB 조회
        ...
        # [★] 비율 계산은 '총 키워드 히트 수' (len(matched_details)) 기준
        total_keyword_hits = len(matched_details) 
        if total_keyword_hits < 7:
             raise HTTPException(status_code=400, detail=f"분석 불가: 매칭된 성분이 {total_keyword_hits}개로 너무 적습니다. (최소 7개 필요)")
        ...
        # 7. JSON 반환 (product_info와 ingredients 수정)
        return {
            "product_info": {
                "name": product.get('product_name', 'N/A'),
                "category": product.get('category', 'N/A'),
                "total_count": total_count,
                "matched_count": unique_matched_count # [★] '고유 성분 수'로 변경
            },
            ...
            "ingredients": { 
                ...
                "unmatched": unmatched # [★] 'unmatched_count' -> 'unmatched' 리스트 전체
            }
        }
