...
import { User, Mail, Calendar, Edit2, Save, Menu, X, Home, MessageSquare, UserCircle, Star, Clock, Bookmark, Plus, Trash2, AlertTriangle, Sparkles, Camera, Settings as SettingsIcon, Bell, LayoutDashboard, TrendingUp } from "lucide-react";
...
export default function UserProfile({
  ...
  const handleAddIngredient = () => {
    ...
    if (newIngredientType === 'preferred') {
      ...
      setCautionIngredients([...cautionIngredients, {
        name: newIngredient,
        reason: "사용자 추가 성분",
        severity: "medium"
      }]);
    ...
    background: 'linear-gradient(135deg, #fce7f3 0%, #f3e8ff 50%, #ddd6fe 100%)'
        ...
        <div className="container mx-auto px-4 sm:px-6 py-3 sm:py-4">
          <div className="flex items-center justify-between">
            ...
            <nav className="hidden md:flex items-center space-x-1">
              ...
              <button onClick={() => onNavigate?.('profile')} className="flex items-center space-x-2 px-4 py-2 rounded-lg font-medium transition-colors" style={{
      ...
      <main className="container mx-auto px-4 sm:px-6 py-4 sm:py-8 max-w-7xl">
        ...
        duration: 0.5
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 gap-3">
            ...
            {!isEditing ? <button onClick={() => setIsEditing(true)} className="flex items-center space-x-2 px-4 py-2 rounded-lg text-white hover:shadow-lg transition-all text-sm sm:text-base w-full sm:w-auto justify-center" style={{
            background: 'linear-gradient(135deg, #f5c6d9 0%, #e8b4d4 100%)'
          }}>
                <Edit2 className="w-4 h-4" />
                <span>프로필 수정</span>
              </button> : <button onClick={handleSave} className="flex items-center space-x-2 px-4 py-2 rounded-lg bg-green-500 text-white hover:bg-green-600 transition-colors text-sm sm:text-base w-full sm:w-auto justify-center">
            ...
            <div className="flex flex-col md:flex-row gap-6">
              ...
              <div className="flex flex-col items-center md:items-start">
                <div className="relative">
                  ...
                  {isEditing && <button className="absolute bottom-0 right-0 w-10 h-10 bg-white rounded-full shadow-lg flex items-center justify-center text-pink-600 hover:bg-pink-50 transition-colors">
                      <Camera className="w-5 h-5" />
                    </button>}
                </div>
                {isEditing ? <input type="text" value={userData.nickname} onChange={e => setUserData({
                ...userData,
                nickname: e.target.value
              })} className="mt-4 text-xl font-bold text-gray-800 text-center px-3 py-1 border border-purple-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 w-full" /> : <h3 className="mt-4 text-xl font-bold text-gray-800">{userData.nickname}</h3>}
              </div>
              ...
              <div className="flex-1">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    {isEditing ? <input type="text" value={userData.fullName} onChange={e => setUserData({
                    ...userData,
                    fullName: e.target.value
                  })} className="w-full px-4 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-pink-300" /> : <p className="text-sm text-gray-600 px-4 py-2 bg-gray-50 rounded-lg">{userData.fullName}</p>}
                  </div>
                  ...
                  <div>
                    ...
                    {isEditing ? <input type="email" value={userData.email} onChange={e => setUserData({
                    ...userData,
                    email: e.target.value
                  })} className="w-full px-4 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-pink-300" /> : <p className="text-sm text-gray-600 px-4 py-2 bg-gray-50 rounded-lg break-all">{userData.email}</p>}
                  </div>
                  ...
                  <div>
                    ...
                    {isEditing ? <input type="date" value={userData.birthday} onChange={e => setUserData({
                    ...userData,
                    birthday: e.target.value
                  })} className="w-full px-4 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-pink-300" /> : <p className="text-sm text-gray-600 px-4 py-2 bg-gray-50 rounded-lg">
                  ...
                  {/* 피부 고민 */}
                  <div>
                    <label className="text-sm font-semibold text-gray-700 mb-2 block">피부 고민</label>
                    <div className="flex flex-wrap gap-2 px-4 py-2 bg-gray-50 rounded-lg min-h-[44px] items-center">
                      {userData.skinConcerns.map((concern, index) => <span key={index} className="px-3 py-1 bg-pink-100 text-pink-700 rounded-full text-xs font-medium">
                          {concern}
                        </span>)}
                    </div>
                  </div>
        ...
        <div className="bg-white rounded-t-2xl shadow-lg mb-0">
          <div className="flex border-b border-gray-200">
            <button onClick={() => setActiveTab('activity')} className={`flex-1 py-3 sm:py-4 px-4 sm:px-6 text-sm sm:text-base font-semibold transition-colors ${activeTab === 'activity' ? 'text-pink-600 border-b-2 border-pink-600' : 'text-gray-500 hover:text-gray-700'}`}>
              나의 활동
            </button>
            <button onClick={() => setActiveTab('ingredients')} className={`flex-1 py-3 sm:py-4 px-4 sm:px-6 text-sm sm:text-base font-semibold transition-colors ${activeTab === 'ingredients' ? 'text-pink-600 border-b-2 border-pink-600' : 'text-gray-500 hover:text-gray-700'}`}>
              성분 관리
            </button>
          </div>
        </div>
          ...
          {activeTab === 'activity' ? <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
                ...
                <div className="bg-purple-50 rounded-xl p-4 border border-purple-100">
                  ...
                  <div className="space-y-3">
                    {recentIngredients.map((ingredient, index) => <div key={index} className="bg-white rounded-lg p-3 shadow-sm">
                ...
                <div className="bg-blue-50 rounded-xl p-4 border border-blue-100">
                  ...
                  <div className="space-y-3">
                    {favoriteProducts.map(product => <div key={product.id} className="bg-white rounded-lg p-3 shadow-sm">
                ...
                <div className="bg-pink-50 rounded-xl p-4 border border-pink-100">
                  ...
                  <div className="space-y-3">
                    {recentRecommendations.map(recommendation => <div key={recommendation.id} className="bg-white rounded-lg p-3 shadow-sm">
              ...
              <div className="bg-gradient-to-r from-pink-50 to-purple-50 rounded-xl p-4 sm:p-6 border border-pink-200">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <input type="text" value={newIngredient} onChange={e => setNewIngredient(e.target.value)} placeholder="성분 이름을 입력하세요" className="flex-1 px-4 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-pink-300" />
                  <select value={newIngredientType} onChange={e => setNewIngredientType(e.target.value as 'preferred' | 'caution')} className="px-4 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-pink-300">
                    <option value="preferred">선호 성분</option>
                    <option value="caution">주의 성분</option>
                  </select>
                  <button onClick={handleAddIngredient} className="px-6 py-2 rounded-lg text-white font-medium hover:shadow-lg transition-all text-sm" style={{
                background: 'linear-gradient(135deg, #f5c6d9 0%, #e8b4d4 100%)'
              }}>
                    추가
                  </button>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                ...
                <div className="bg-green-50 rounded-xl p-4 sm:p-6 border border-green-200">
                  ...
                  <div className="space-y-3">
                    {preferredIngredients.map((ingredient, index) => <motion.div key={index} initial={{
                  opacity: 0,
                  x: -20
                }} animate={{
                  opacity: 1,
                  ...
                  delay: index * 0.05
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1 min-w-0">
                            <h4 className="text-sm sm:text-base font-bold text-gray-800 mb-1">{ingredient.name}</h4>
                            <p className="text-xs sm:text-sm text-gray-600">{ingredient.benefit}</p>
                          </div>
                          <button onClick={() => removePreferredIngredient(index)} className="ml-3 text-red-500 hover:text-red-700 transition-colors flex-shrink-0">
                ...
                <div className="bg-red-50 rounded-xl p-4 sm:p-6 border border-red-200">
                  ...
                  <div className="space-y-3">
                    {cautionIngredients.map((ingredient, index) => <motion.div key={index} initial={{
                  opacity: 0,
                  x: -20
                }} animate={{
                  opacity: 1,
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1 min-w-0">
                            <div className="flex flex-wrap items-center gap-2 mb-1">
                              <h4 className="text-sm sm:text-base font-bold text-gray-800">{ingredient.name}</h4>
                              <span className={`px-2 py-0.5 rounded-full text-xs font-semibold uppercase ${getSeverityBadge(ingredient.severity)}`}>
                                {ingredient.severity}
                              </span>
                            </div>
                            <p className="text-xs sm:text-sm">{ingredient.reason}</p>
                          </div>
                          <button onClick={() => removeCautionIngredient(index)} className="ml-3 text-red-500 hover:text-red-700 transition-colors flex-shrink-0">
                            <Trash2 className="w-4 h-4" />
                          </button>
                        </div>
                      </motion.div>)}
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-lg border-t border-pink-100 z-50">
        <div className="flex items-center justify-around px-4 py-3">
          ...
          <button onClick={() => onNavigate?.('profile')} className="flex flex-col items-center space-y-1 text-pink-600">
