...
import {
  User, Mail, Calendar, Edit2, Save, Menu, X,
  Home, MessageSquare, UserCircle, Star, Clock,
  Bookmark, Plus, Trash2, AlertTriangle, Sparkles,
  Camera, Settings as SettingsIcon, Bell, LayoutDashboard,
  TrendingUp,
  Loader2, // [★] 저장 로딩 아이콘
  Heart, // [★] 성별 아이콘 (선택)
} from 'lucide-react';
      ...
      {bubbles.map((bubble) => (
        <motion.div
          ...
          style={{
            ...
            background: 'radial-gradient(circle at 30% 30%, rgba(255, 255, 255, 0.9), rgba(248, 215, 230, 0.7), rgba(232, 180, 212, 0.5))',
            boxShadow: 'inset -10px -10px 30px rgba(255, 255, 255, 0.8), inset 5px 5px 20px rgba(248, 215, 230, 0.5), 0 0 30px rgba(248, 215, 230, 0.4)',
        ...
        <div className="container mx-auto px-4 sm:px-6 py-3 sm:py-4">
          <div className="flex items-center justify-between">
            ...
            <nav className="hidden md:flex items-center space-x-1">
              ...
              <button onClick={() => onNavigate?.('profile')} className="flex items-center space-x-2 px-4 py-2 rounded-lg font-medium transition-colors" style={{ background: 'linear-gradient(135deg, #f5c6d9 0%, #e8b4d4 100%)', color: 'white' }}>
            ...
            <div className="hidden md:flex items-center space-x-4">
              ...
              <button onClick={() => onNavigate?.('profile')} className="w-10 h-10 rounded-full flex items-center justify-center text-white font-semibold" style={{ background: 'linear-gradient(135deg, #f5c6d9 0%, #e8b4d4 100%)' }}>
                ...
                {(userData.nickname || userData.name || 'U').charAt(0).toUpperCase()}
              </button>
            </div>
      ...
      <main className="container mx-auto px-4 sm:px-6 py-4 sm:py-8 max-w-7xl">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="mb-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 gap-3">
            ...
            {!isEditing ? (
              <button onClick={() => setIsEditing(true)} className="flex items-center space-x-2 px-4 py-2 rounded-lg text-white hover:shadow-lg transition-all text-sm sm:text-base w-full sm:w-auto justify-center" style={{ background: 'linear-gradient(135deg, #f5c6d9 0%, #e8b4d4 100%)' }}>
                <Edit2 className="w-4 h-4" /> <span>프로필 수정</span>
              </button>
            ) : (
              <button onClick={handleSave} disabled={isSaving} className="flex items-center space-x-2 px-4 py-2 rounded-lg bg-green-500 text-white hover:bg-green-600 transition-colors text-sm sm:text-base w-full sm:w-auto justify-center min-w-[140px] disabled:opacity-70">
                {isSaving ? <WaterDropletLoader /> : (
            ...
            <div className="flex flex-col md:flex-row gap-6">
              <div className="flex flex-col items-center md:items-start">
                <div className="relative">
                  <div className="w-32 h-32 rounded-full flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #f5c6d9 0%, #e8b4d4 100%)' }}>
                ...
                {isEditing ? (
                  ...
                  <h3 className="mt-4 text-xl font-bold text-gray-800">{userData.nickname || userData.name || '(방문자)'}</h3>
                )}
              </div>
              ...
              <div className="flex-1">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="text-sm font-semibold text-gray-700 mb-2 block">이름 (수정 불가)</label>
                  ...
                  <div>
                    ...
                    {isEditing ? (
                      ...
                      <p className="text-sm text-gray-600 px-4 py-2 bg-gray-50 rounded-lg">
                        {userData.gender === 'female' ? '여성' : 
                         userData.gender === 'male' ? '남성' : 
                         userData.gender === 'other' ? '기타' : '(미입력)'}
                      </p>
                    )}
                  </div>
                    ...
                    <div className="flex flex-wrap gap-2 px-4 py-2 bg-gray-50 rounded-lg min-h-[44px] items-center">
                      {isEditing ? (
                        <input
                          ...
                          onChange={e => setUserData({ ...userData, skinType: e.target.value.toUpperCase() })}
                          placeholder="예: OSNT"
                          maxLength={4}
            ...
            <div className="space-y-6">
              ...
              <div className="bg-gradient-to-r from-pink-50 to-purple-50 rounded-xl p-4 sm:p-6 border border-pink-200">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  ...
                  <button onClick={handleAddIngredient} className="px-6 py-2 rounded-lg text-white font-medium hover:shadow-lg transition-all text-sm" style={{ background: 'linear-gradient(135deg, #f5c6d9 0%, #e8b4d4 100%)' }}>
                    추가
                  </button>
                </div>
              </div>
                ...
                <div className="bg-green-50 rounded-xl p-4 sm:p-6 border border-green-200">
                  ...
                  <div className="space-y-3">
                    {preferredIngredients.map((ingredient, index) => (
                      <motion.div key={index} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3, delay: index * 0.05 }} className="bg-white rounded-lg p-3 sm:p-4 border-2 border-green-200 hover:shadow-md transition-shadow">
                ...
                <div className="bg-red-50 rounded-xl p-4 sm:p-6 border border-red-200">
                  ...
                  <div className="space-y-3">
                    {cautionIngredients.map((ingredient, index) => (
                      <motion.div key={index} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3, delay: index * 0.05 }} className={`rounded-lg p-3 sm:p-4 border-2 hover:shadow-md transition-shadow ${getSeverityColor(ingredient.severity)}`}>
                        <div className="flex items-start justify-between">
                          <div className="flex-1 min-w-0">
