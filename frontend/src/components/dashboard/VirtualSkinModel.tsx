...
export default function VirtualSkinModel({ skinType }: VirtualSkinModelProps) {
  // --- 상태 관리 (기존과 동일) ---
          ...
          {!isSimLoading && analysisResult && (
            <div className="flex flex-col items-center text-center">
              ...
              {analysisResult?.meta?.reliability === 'low' && (
                <div className="mb-2 rounded-md border border-yellow-200 bg-yellow-50 px-2 py-1 text-[12px] inline-flex items-center gap-1">
                  <span>저신뢰 분석: OCR 매칭 성분이 적습니다. 결과 해석에 유의하세요.</span>
                  <button
                    type="button"
                    className="ml-1 inline-flex h-4 w-4 items-center justify-center rounded-full border border-yellow-300 bg-yellow-100 text-[10px] leading-none"
