...
- [OCR 기반 제품 인식 및 기능 연계](#-ocr-기반-제품-인식-및-기능-연계)
- [제품 피부 적합도 계산](#-제품-피부-적합도-계산)
...
사용자가 업로드한 제품 이미지에서 Google Cloud Vision OCR로 텍스트를 추출하고 제품명 또는 성분 정보를 식별합니다. AI 채팅은 일반 검색과 분리된 OCR API에서 제품 DB를 대조해 제품·성분 분석 응답을 만들며, 개인 대시보드는 별도의 OCR API에서 식별한 성분을 피부 타입 기반 적합도 계산에 전달합니다.

```mermaid
flowchart TD
    A["제품 이미지 입력"] --> B{"사용 기능"}
    B -->|"AI 채팅"| C["Google Cloud Vision OCR"]
    C --> D["텍스트 정제와 제품명 후보 추출"]
    D --> E["FTS 후보 검색 후 문자열 유사도 비교"]
    E --> F{"유사도 0.6 이상 제품 매칭"}
    F -->|"성공"| G["DB 제품 정보와 전성분 조회"]
    F -->|"실패"| H["제품명 후보별 LIKE 검색"]
    H --> I{"제품 매칭"}
    I -->|"성공"| G
    I -->|"실패"| J["OCR 텍스트에서 성분 직접 추출"]
    G --> K["제품과 주의 성분 분석"]
    J --> K
    K --> L["채팅 화면에 별도 OCR 분석 응답"]
    B -->|"개인 대시보드"| M["Google Cloud Vision OCR"]
    M --> N["텍스트 토큰화와 성분 후보 정규화"]
    N --> O["KCIA와 성분 DB 대조"]
    O --> P["피부 타입 기반 적합도 계산"]
    P --> Q["점수와 신뢰도 및 주의 성분 결과"]
```

- AI 채팅은 `/api/ocr/analyze-image`에서 문서 OCR 결과의 앞부분을 이용해 제품명 후보를 만들고, FTS 후보의 문자열 유사도를 비교한 뒤 후보별 `LIKE` 검색을 수행합니다. 제품을 찾지 못하면 OCR 텍스트에서 성분을 직접 추출합니다.
- 개인 대시보드는 `/api/analyze-ocr`에서 OCR 텍스트를 토큰화해 KCIA·성분 DB와 대조하며, 제품명 검색 없이 확인된 성분을 적합도 계산에 사용합니다.
- 두 경로는 Google Cloud Vision을 사용하지만 서로 다른 API와 OCR 처리 로직으로 구현되어 있습니다.

## 🎯 제품 피부 적합도 계산

바우만 설문 결과로 사용자를 16가지 피부 타입 중 하나로 분류하고, 제품 성분 구성과 타입별 기준을 비교해 0~100점의 상대적 적합도를 제공합니다. 개인 대시보드에서 직접 선택한 제품의 `/api/analyze`와 이미지로 분석한 제품의 `/api/analyze-ocr` 결과에 사용됩니다.

| 계산 단계 | 적용 기준 |
| --- | --- |
| 성분 정규화 | 제품 전성분을 정규화하고 보습·진정·피지·주름·미백·보호의 6가지 효능군으로 분류합니다. |
| 효능 비율 계산 | 전체 성분 수가 아닌 전체 효능 매칭 횟수를 분모로 각 효능군의 상대적 비율을 계산합니다. |
| 피부 타입 기준 적용 | 16가지 피부 타입별 `importance`, `target_min`, `target_max`를 적용합니다. |
| 목표 범위 평가 | 효능 비율이 목표 범위 안이면 중앙에서 0.97, 경계에서 0.90이 되도록 차등 평가합니다. |
| 감점 기여도 반영 | 음수 `importance`는 효능 비율이 목표 상한을 초과한 경우에만 감점 기여도로 반영합니다. |
| 기여도 정규화 | 효능별 기여도 합계를 피부 타입별 최대 양수 기여도와 최소 음수 기여도 범위로 정규화합니다. |
| 점수 합산 | 기본 점수 25점과 정규화된 성과 점수 75점을 합산하고 올림 처리합니다. |
| 분석 신뢰도 반영 | 직접 선택 제품과 OCR 분석에서 효능 매칭이 3개 미만이면 분석을 중단하고, 3~6개는 75점, 7~9개는 95점으로 최종 점수 상한을 적용합니다. |
| 주의 성분 반영 | 시스템 주의 성분은 결과에 표시하고, 사용자 지정 주의 성분이 포함되면 40점을 감점합니다. |
| 최종 점수 제한 | 감점까지 반영한 최종 결과를 0~100점으로 제한합니다. |

위 신뢰도 중단 및 점수 상한 기준은 직접 선택 제품의 `/api/analyze`와 OCR 분석의 `/api/analyze-ocr`에 적용됩니다.
