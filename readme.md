...
| 🎯 **제품 피부 적합도 계산** | 제품 성분을 6가지 효능군으로 분류해 전체 효능 매칭 횟수 대비 상대 비율을 구하고, 피부 타입별 중요도·목표 범위와 분석 신뢰도·사용자 지정 주의 성분을 반영한 0~100점의 상대적 적합도를 제공합니다. |
| 📷 **OCR 기반 제품 인식 및 기능 연계** | Google Cloud Vision OCR을 AI 채팅과 개인 대시보드의 공통 이미지 입력 기술로 사용합니다. AI 채팅은 제품명 후보와 성분을 DB에 대조해 별도 분석 응답을 제공하고, 대시보드는 OCR 성분을 피부 타입 기반 적합도 계산에 전달합니다. |
| 📊 **상품·리뷰 추이 시각화** | 상품별 리뷰 수 변화와 인기 상품 정보를 시각화합니다. |

<details>
<summary><strong>제품 피부 적합도 계산 기준</strong></summary>

- 제품 전성분을 정규화한 뒤 보습·진정·피지·주름·미백·보호의 6가지 효능군으로 분류하고, 전체 효능 매칭 횟수에서 각 효능군이 차지하는 상대적 비율을 계산합니다.
- 16가지 피부 타입별 `importance`, `target_min`, `target_max`를 적용합니다. 목표 범위 안에서는 중앙에 가까울수록 0.97, 경계에 가까울수록 0.90의 적합도를 부여하며, 음수 중요도는 목표 상한을 초과한 경우에만 감점 기여도로 반영합니다.
- 효능별 기여도 합계를 피부 타입별 최대 양수 기여도와 최소 음수 기여도 범위로 정규화하고, 기본 점수 25점과 정규화 성과 점수 75점을 합산합니다.
- 제품 상세·OCR 분석에서는 효능 매칭이 3개 미만이면 분석을 중단하고, 3~6개는 75점, 7~9개는 95점으로 점수 상한을 적용합니다.
- 시스템 주의 성분은 결과에 표시하며, 사용자 지정 주의 성분이 포함되면 40점을 감점한 뒤 최종 점수를 0~100점으로 제한합니다.

이 점수는 프로젝트에서 정의한 피부 타입별 기준으로 제품의 성분 구성을 상대적으로 평가한 값이며, 의학적 진단이나 피부 개선 효과를 보장하지 않습니다.
