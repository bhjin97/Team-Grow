...
제품별 공감순 상위 30개 리뷰에서 반복적으로 언급되는 특징과 사용 경험을 LLM으로 요약했습니다. 요약 결과를 임베딩 모델로 벡터화해 Pinecone에 적재하고 의미 기반 제품 검색에 활용했습니다.
...
### 1. 크롤링 속도와 실행 안정성 사이의 균형 조정

* **문제:** 초기에는 AWS EC2에서 제품·리뷰 크롤링을 실행하려 했지만, 동일한 코드가 로컬 환경에서는 작동하는 반면 클라우드 환경에서는 대상 사이트에 정상적으로 접근하지 못했습니다. 로컬 환경으로 실행 위치를 변경한 뒤에는 하나의 크롤링 세션이 처리하는 범위가 크면 자원 부하로 작업이 중단되었고, 세션을 여러 개로 나누어 병렬 실행하면 요청이 집중되어 대상 사이트의 접근 제한이 발생했습니다.

* **해결:** 크롤링 세션 수를 줄이는 대신 로컬 환경이 감당할 수 있는 범위까지 세션당 처리량을 늘렸습니다. 또한 여러 세션을 병렬로 실행하지 않고 순차적으로 처리하도록 실행 구조를 변경했으며, Playwright의 Headless 모드를 비활성화해 실제 브라우저와 유사한 환경에서 작업을 수행했습니다.

* **결과:** 처리 속도만을 우선하는 병렬 실행 방식에서 벗어나 로컬 자원 사용량과 대상 사이트의 접근 제한을 함께 고려한 안정적인 크롤링 방식을 구축했습니다. 수집한 제품·리뷰 데이터는 정제와 Upsert 과정을 거쳐 운영 데이터베이스에 반영했습니다.

---

### 2. 복합 질의를 처리하기 위한 검색 라우팅과 성분 데이터 표준화

- **문제:** 사용자의 질문에는 제품의 사용감·효과 같은 자연어 특징과 브랜드·가격·카테고리·성분 조건이 함께 포함될 수 있어, 하나의 검색 방식만으로 의미적 유사성과 정확한 조건을 모두 처리하기 어려웠습니다.

- **해결:** LLM으로 질문을 일반 정보형과 제품 검색형으로 분류하고 브랜드·성분·특징·가격 조건을 JSON 형태로 추출했습니다. 카테고리는 동의어 사전과 문자열 규칙으로 보정했으며, 추출된 조건 조합에 따라 Vector-first, RDB-first 또는 RDB-only 검색 경로를 선택하도록 LangChain 기반 라우팅을 구성했습니다. 제품 특징은 Pinecone에서 의미 검색하고, 브랜드·가격·카테고리·성분 조건은 MariaDB에서 적용했습니다.

- **결과:** 자연어 특징이 중심인 질문은 Vector DB에서 유사 후보를 먼저 검색하고, 정확한 조건이 충분한 질문은 MariaDB에서 후보를 먼저 축소했습니다. 이를 통해 검색 정확도를 유지하면서 불필요한 벡터 비교와 데이터 조회를 줄이고, 최종 LLM에 전달되는 컨텍스트의 크기를 제한했습니다.
