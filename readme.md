...
* **문제:** 사용자의 질문에는 제품 특징뿐 아니라 브랜드·가격·카테고리·성분 조건이 다양하게 조합될 수 있어, 모든 질문을 하나의 검색 방식으로 처리하기 어려웠습니다. 또한 상품 테이블의 성분명이 제품마다 다르게 표기되거나 축약되어 있어 성분 테이블의 표준 성분명과 정확하게 연결되지 않았습니다.

* **해결:** 예상 질문을 시나리오별로 분류하고, 질문에 포함된 조건에 따라 RDB와 Vector DB 중 필요한 데이터 소스와 검색 순서를 선택하도록 라우팅 구조를 설계했습니다. 일부 시나리오는 LangChain과 LLM을 이용해 질문을 분류하고 적절한 검색 경로로 전달하도록 구현했습니다. 성분 검색을 지원하기 위해 성분 테이블의 성분명을 기준으로 동일·유사·축약 성분명을 조합해 표준화하고, 총 192,399건의 데이터를 적재한 성분명 매핑 테이블을 구축했습니다.

* **결과:** 자연어로 표현된 제품 특징은 벡터 검색으로 탐색하고, 브랜드·가격·카테고리·성분처럼 명시적인 조건은 RDB에서 확인할 수 있는 Hybrid RAG 검색 기반을 마련했습니다. 또한 일관되지 않던 상품별 성분명을 표준 성분명과 연결하여 성분 조건이 포함된 질문을 검색할 수 있도록 데이터 구조를 개선했습니다.
